...
if [ $? -eq 0 ]; then
    ...
    # Set the certificate ID for the various platforms
    echo "Next export the name of the created certificate to relevant environment variables:"
    echo "export MACOS_CERTID=$CERT"
    echo "export IOS_CERTID=$CERT"
    echo "export WATCHOS_CERTID=$CERT"
    echo "export TVOS_CERTID=$CERT"
    exit 0
fi
